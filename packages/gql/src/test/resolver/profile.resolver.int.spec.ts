...
describe('profile resolver', () => {
  ...
  describe('fullFillIncentiveActions', () => {
    beforeAll(async () => {
      testMockUser.chainId = '5'
      testMockWallet.chainId = '5'
      testMockWallet.chainName = 'goerli'
    ...
    it('should full fill incentive actions for profiles', async () => {
      const result = await testServer.executeOperation({
        query: 'mutation FullFillIncentiveActions($input: FullFillIncentiveActionsInput!) { fullFillIncentiveActions(input:$input) {  message } }',
        variables: {
          input: {
            take: 2,
            skip: 0,
      ...
      expect(result.data.fullFillIncentiveActions.message).toEqual('Full filled incentive actions for 2 profiles')
      const actions = await repositories.incentiveAction.findAll()
