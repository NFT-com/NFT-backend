...
const getAddressFromSignature = (signature: string): string => utils.verifyMessage(authMessage, signature)

export const authMiddleWare = (_req, res, next): any => {
  const authSignature = _req.headers[authHeader] || null

  if (!authSignature) {
    return res.status(401).send(buildError(AuthErrorType.Unauthenticated, 'Not authenticated!'))
  }
  ...
  if (!allowedList.includes(address)) {
    return res.status(403).send(buildError(AuthErrorType.Forbidden, 'Not allowed to perform this operation!'))
  }
