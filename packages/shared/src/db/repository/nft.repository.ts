...
export class NFTRepository extends BaseRepository<NFT> {
  ...
  findAllWithRelations(): Promise<NFT[]> {
    return this.getRepository(true)
      ...
      .leftJoinAndMapOne('nft.collection', Collection, 'collection', 'nft.contract = collection.contract')
      .leftJoinAndMapOne('nft.wallet', Wallet, 'wallet', 'nft.walletId = wallet.id')
      .getMany()
  }
  ...
  findDistinctContracts(): Promise<any[]> {
    return this.getRepository(true).createQueryBuilder('nft').select('nft.contract').distinct(true).getRawMany()
  }

  findNFTsWithPreviewLinks(): Promise<NFT[]> {
    return this.getRepository(true).createQueryBuilder('nft').where('nft.previewLink is not null').getMany()
  }
