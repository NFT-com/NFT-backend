...
export const mapContractsToSlugs = async (contracts: DistinctContract[]): Promise<string[]> => {
  ...
  const checksumContracts: string[] = contracts.map((contract: DistinctContract) =>
    helper.checkSum(contract?.nft_contract),
  )
  // caching strategy
  for (const contract of checksumContracts) {
    const existingKey: string[] = cachedMembers.filter((cachedMember: string) => cachedMember.includes(contract))
    const keyExists = Boolean(existingKey?.length && existingKey?.[0]?.split(':')?.[0] === contract)
    if (!keyExists) {
  ...
  const slugsToRemove: string[] = cachedMembers.filter(
    (cachedMember: string) => !checksumContracts.includes(cachedMember.split(':')?.[0]),
  )
                  ...
                  if (nft) {
                    ...
                    if (orderHash) {
                      ...
                      if (!order) {
                        try {
                          logger.debug(
                            `[seaport-streams] order with orderHash: ${orderHash} for ${nftId}: eventPayload: ${JSON.stringify(
                              eventPayload,
                            )}`,
                          )
                          ...
                          logger.debug(
                            `order with orderHash: ${orderHash} for ${nftId} is saved successfully on ${network} network, newOrder: ${JSON.stringify(
                              newOrder,
                            )}`,
                          )
                        } catch (err) {
