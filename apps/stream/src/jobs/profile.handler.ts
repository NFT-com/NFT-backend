...
export const nftUpdateBatchProcessor = async (job: Job): Promise<boolean> => {
  ...
  try {
    ...
    await Promise.allSettled(
      nfts.map(async nft => {
        const savedNFT = await nftService.updateNFTOwnershipAndMetadata(nft, userId, wallet, chainId)
...
const updateWalletNFTs = async (profileUrl: string, chainId: string): Promise<void> => {
  try {
    ...
    if (!profile) {
      await removeProfileIdFromRelevantKeys(ProfileCacheEnum.WALLET_NFTS, profile.url, chainId)
      ...
      if (inProgress) {
        ...
        if (inProgressScore > PROFILE_PROGRESS_THRESHOLD) {
          if (failScore > inProgressScore) {
            logger.log(
              { profile },
              `Profile stuck in progress longer than expected: profile ${profile.url} (${profile.id}), fail_score: ${failScore}`,
            )
          ...
          logger.log(
            `Threshold crossed ${failScore + 1} times for profile ${profile.url} (${
              profile.id
            }) - current progress score: ${inProgressScore}`,
          )
        } else {
        ...
        logger.info(
          `[updateWalletNFTs-4] starting nftService.updateWalletNFTs ${profile.url} (${profile.id}), ${getTimeStamp(
            start,
          )}`,
        )
        ...
        if (profile.ownerWalletId) {
          ...
          logger.log(
            `[updateWalletNFTs-13] Owner wallet Id is null for profile id: ${profile.id} and url: ${profile.url}`,
          )
          await removeProfileIdFromRelevantKeys(ProfileCacheEnum.WALLET_NFTS, profile.url, chainId)
          await removeProfileIdFromRelevantKeys(ProfileCacheEnum.PROFILE_OWNER, profile.url, chainId)
        }

        logger.info(
          `[updateWalletNFTs-14] completed updating NFTs for profile ${profile.url} (${
            profile.id
          }), TOTAL: ${getTimeStamp(constantStart)}`,
        )
      }
    }
  } catch (err) {
...
const processProfileUpdate = async (profileUrl: string, chainId: string): Promise<void> => {
  ...
  const profile = await repositories.profile.findOne({ where: { url: profileUrl } })
  if (!profile) {
    await removeProfileIdFromRelevantKeys(ProfileCacheEnum.PROFILE_OWNER, profileUrl, chainId)
    ...
    if (inProgress) {
      ...
      if (profile.ownerWalletId) {
        ...
        if (!wallet) {
          await removeProfileIdFromRelevantKeys(ProfileCacheEnum.PROFILE_OWNER, profile.url, chainId)
          logger.info(
            `4. [processProfileUpdate] No wallet found for ID ${profile.ownerWalletId} (url = ${profile.url})`,
          )
        } else {
          try {
            logger.info(
              `5. [processProfileUpdate] Updating NFTs for profile ${profile.url} (${profile.id}), ${getTimeStamp(
                start,
              )}`,
            )
            start = new Date().getTime()
            ...
            nftService.initiateWeb3(chainId)
            await nftService.checkNFTContractAddresses(profile.ownerUserId, wallet.id, wallet.address, chainId)
            logger.info(
              `6. [processProfileUpdate] checked NFT contract addresses for profile ${profile.url} (${
                profile.id
              }), ${getTimeStamp(start)}`,
            )
            start = new Date().getTime()
            ...
            logger.info(
              `7. [processProfileUpdate] syncEdgesWithNFTs for profile ${profile.url} (${profile.id}), ${getTimeStamp(
                start,
              )}`,
            )
            ...
            await cache.del([
              `${CacheKeys.PROFILE_SORTED_NFTS}_${chainId}_${profile.id}`,
              `${CacheKeys.PROFILE_SORTED_VISIBLE_NFTS}_${chainId}_${profile.id}`,
            ])
            await Promise.all([
              cache.zadd(`${CacheKeys.UPDATED_NFTS_PROFILE}_${chainId}`, ttl, profile.url),
              removeProfileIdFromRelevantKeys(ProfileCacheEnum.PROFILE_OWNER, profile.url, chainId),
            ])
          } catch (err) {
        ...
        logger.log(
          `[processProfileUpdate] Owner wallet Id is null for profile id: ${profile.id} and url: ${profile.url}`,
        )
        await removeProfileIdFromRelevantKeys(ProfileCacheEnum.PROFILE_OWNER, profile.url, chainId)
        await removeProfileIdFromRelevantKeys(ProfileCacheEnum.WALLET_NFTS, profile.url, chainId)
      }
  ...
  try {
    ...
    for (const profileUrl of cachedProfiles) {
      ...
      if (profile) {
        ...
        processProfileUpdate(profileUrl, chainId).catch(err => logger.error(err))
      } else {
        logger.info(
          `[updateNFTsOwnershipForProfilesHandler] Profile not found for url ${profileUrl}, chainId=${chainId}`,
        )
        continue
      }
    }
...
export const pullNewNFTsHandler = async (job: Job): Promise<any> => {
  ...
  try {
    ...
    // 2. update NFTs for profiles cached in UPDATE_NFTS_PROFILE cache
    const cachedProfiles = await cache.zrevrangebyscore(
      `${CacheKeys.UPDATE_WALLET_NFTS_PROFILE}_${chainId}`,
      '+inf',
      '(0',
    )
      ...
      if (!inProgress) {
        ...
        if (profile) {
          ...
          if (estimateNftsCount < MAX_NFTS_PER_PROFILE) {
            logger.info(
              `2b. [pullNewNFTsHandler] Updating NFTs for profile ${profile.url} ==> (estimateNftsCount = ${estimateNftsCount})`,
            )
            ...
            logger.info(
              `2c. [pullNewNFTsHandler] Too many NFTs for profile ${profile.url} ==> (estimateNftsCount = ${estimateNftsCount} > ${MAX_NFTS_PER_PROFILE})`,
            )
          }
        } else {
...
export const profileGKOwnersHandler = async (job: Job): Promise<any> => {
  try {
    ...
    for (const profile of profiles) {
      ...
      const isIncludedInGKOwners: boolean =
        profileWallet?.address && ownersOfGK[helper.checkSum(profileWallet?.address)]

      if (isIncludedInGKOwners) {
        if (!profile?.gkIconVisible || profile.gkIconVisible === null || profile.gkIconVisible === undefined) {
      ...
      if (profileUpdatePromise.length > 50) {
        ...
        if (cacheProfiles.length) {
          logger.info(
            `[profileGKOwnersHandler-1] Sync profile gk owners cacheProfiles: ${JSON.stringify(cacheProfiles)}`,
          )
    ...
    for (const walletId of cachedWallets) {
      if (walletId) {
        ...
        if (wallet && wallet.userId && !profile) {
          try {
            logger.log(
              `[updateNFTsForNonProfilesHandler]: Wallet id being processed: ${walletId}, profile: ${profile}, ${getTimeStamp(
                start,
              )}}`,
            )
            start = new Date().getTime()
            ...
            logger.log(
              `[updateNFTsForNonProfilesHandler]: Finished processing wallet id: ${walletId}, ${getTimeStamp(start)}}`,
            )
            start = new Date().getTime()
          } catch (err) {
            logger.error(
              err,
              `[updateNFTsForNonProfilesHandler]: Error in updateWalletNFTs while processing wallet: ${walletId}`,
            )
          }
        } else {
          if (!wallet.userId) {
          ...
          if (profile) {
            logger.log(
              `[updateNFTsForNonProfilesHandler]: Wallet Id: ${walletId} has at least one profile ${profile.id} - url: ${profile.url}`,
            )
            await Promise.all([
...
// helper fn
async function doUpdateWalletWork(chainId: string, profile: any, wallet: any, start: number): Promise<void> {
  ...
  logger.info(
    `[updateWalletNFTs-7] nftService.createEdgesForProfile ${profile.url} (${profile.id}), ${getTimeStamp(start)}`,
  )
  start = new Date().getTime()

  await nftService.saveVisibleNFTsForProfile(profile.id, repositories)
  logger.info(
    `[updateWalletNFTs-8] saved amount of visible NFTs and score for profile ${profile.url} (${
      profile.id
    }), ${getTimeStamp(start)}`,
  )
  start = new Date().getTime()
  ...
  let msg = await nftService.updateNFTsForAssociatedAddresses(repositories, profile, chainId)
  logger.info(`[updateWalletNFTs-10] after updateNFTsForAssociatedAddresses ${msg}, ${getTimeStamp(start)}`)
  start = new Date().getTime()

  msg = await nftService.updateCollectionForAssociatedContract(repositories, profile, chainId, wallet.address)
  ...
  if (profile.gkIconVisible) {
    ...
    logger.info(
      `[updateWalletNFTs-12] gkIconVisible updated for profile ${profile.url} (${profile.id}), ${getTimeStamp(start)}`,
    )
    start = new Date().getTime()
  }
  ...
  await Promise.all([
    ...
    removeProfileIdFromRelevantKeys(ProfileCacheEnum.WALLET_NFTS, profile.url, chainId),
  ])
  logger.info(
    `[updateWalletNFTs-doUpdateWalletWork] completed updating NFTs for profile ${profile.url} (${
      profile.id
    }), TOTAL: ${getTimeStamp(begin)}`,
  )
}
