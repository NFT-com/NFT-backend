...
import { getCollectionDeployer } from './alchemy.service'
...
import { NFTPortNFT, retrieveNFTDetailsNFTPort } from './nftport.service'
import { getOpenseaInterceptor } from './opensea.service'
import { SearchEngineService } from './searchEngine.service'
import { paginatedActivitiesBy } from './txActivity.service'
...
type OfficialCollectionNFTsInput = {
  ...
  offsetPageInput?: InputMaybe<pagination.OffsetPageInput>
}
...
export const getNFTsForOfficialCollection = async ({
  ...
  collectionAddress,
  offsetPageInput,
}: OfficialCollectionNFTsInput): Promise<OfficialCollectionNFTsOutput> => {
...
export const getNFTActivities = <T>(activityType: defs.ActivityType) => {
  return async (parent: T, args: unknown, ctx: Context): Promise<Pageable<entity.TxActivity> | null> => {
    try {
      let pageInput: pagination.PageInput = args?.['listingsPageInput']
      const expirationType: any = args?.['listingsExpirationType']
      ...
      const protocol: defs.ProtocolType = args?.['protocol']
      ...
      if (contract && tokenId) {
        ...
        if (!expirationType || expirationType === ActivityExpiration.Active) {
          filters = { ...filters, expiration: helper.moreThanDate(new Date().toString()) }
        } else if (expirationType === ActivityExpiration.Expired) {
