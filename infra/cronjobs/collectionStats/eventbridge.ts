...
const createEventBridgeRule = (): aws.cloudwatch.EventRule => {
  ...
  return new aws.cloudwatch.EventRule('collectionStats-eventRule', {
    name: resourceName,
    scheduleExpression: isProduction() ? 'cron(00 05 * * ? *)' : 'cron(15 05 1 * ? *)',  // if prod, run daily at 05:00 UTC, otherwise at 05:15
    tags: getTags(tags),
  })
}
...
export const createCollectionStatsEventBridgeTarget = (
  ...
  return new aws.cloudwatch.EventTarget('collectionStats-eventTarget', {
    arn: cluster.arn,
    ecsTarget: {
