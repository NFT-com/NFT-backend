...
const splitGetLogs = async (
  ...
  if (totalLogs > MAX_LOGS) {
    logger.info(
      `[splitGetLogs] Exceeded maximum number of logs (${MAX_LOGS}), fromBlock: ${fromBlock}, toBlock: ${toBlock}, totalLogs: ${totalLogs}`,
    )
...
export const getPastLogs = async (
  ...
  if (totalLogs > MAX_LOGS) {
    logger.info(
      `[getPastLogs] Exceeded maximum number of logs (${MAX_LOGS}), fromBlock: ${fromBlock}, toBlock: ${toBlock}, totalLogs: ${totalLogs}`,
    )
    return { logs: [], lastProcessedBlock: toBlock }
  }
  ...
  try {
    ...
    if (toBlock - fromBlock > max_Blocks) {
      ...
      return await splitGetLogs(provider, fromBlock, toBlock, address, topics, max_Blocks, currentStackLv, totalLogs)
    } else {
...
export const getCachedBlock = async (chainId: number, key: string): Promise<number> => {
  const startBlock = chainId == 5 ? 7128515 : chainId == 1 ? 14675454 : 14675454

  try {
    const cachedBlock = await cache.get(key)

    // get 1000 blocks before incase of some blocks not being handled correctly
    if (cachedBlock) return Number(cachedBlock) > 1000 ? Number(cachedBlock) - 1000 : Number(cachedBlock)
...
export const getResolverEvents = async (
  ...
  try {
    ...
    const logs = await getPastLogs(provider, address, topics, cachedBlock, latestBlock.number, Number(maxBlocks))
    return {
      logs: logs.logs,
...
const topics = [helper.id('MintedProfile(address,string,uint256,uint256,uint256,address)')]
...
const topics3 = [[helper.id('ExtendExpiry(string,uint256)'), helper.id('Transfer(address,address,uint256)')]]
...
const syncMintedProfileEvents = async (
  ...
  const log = await getMintedProfileEvents(topics, Number(chainId), chainProvider, profileAuctionAddress)
  ...
  await Promise.allSettled(
    log.logs.map(async unparsedEvent => {
      try {
        ...
        logger.info(
          `minted profile owner=${owner} profileUrl=${profileUrl} tokenId=${BigNumber.from(tokenId).toString()}`,
        )
        ...
        if (evt.name === 'MintedProfile') {
          ...
          if (!existsBool) {
            ...
            if (!profile) {
              ...
              try {
                await core.createProfileFromEvent(chainId.toString(), owner, tokenId, repositories, profileUrl)
                await core.sendSlackMessage(
                  'sub-nftdotcom-analytics',
                  `New profile created: ${profileUrl} by ${owner} (https://www.etherscan.io/tx/${unparsedEvent.transactionHash})`,
                )
              } catch (err) {
...
const syncResolverEvents = async (
  ...
  const log2 = await getResolverEvents(topics2, Number(chainId), chainProvider, nftResolverAddress)
      ...
      try {
        ...
        if (evt.name === EventName.AssociateEvmUser) {
          ...
          if (!event) {
            ...
            logger.info(
              `New NFT Resolver AssociateEvmUser event found. ${profileUrl} (owner = ${owner}) is associating ${destinationAddress}. chainId=${chainId}`,
            )
          }
          ...
          if (!event) {
            ...
            logger.info(
              `New NFT Resolver ${evt.name} event found. ${profileUrl} (owner = ${owner}) is cancelling ${destinationAddress}. chainId=${chainId}`,
            )
          }
          ...
          if (!event) {
            ...
            logger.info(
              `New NFT Resolver ${evt.name} event found. profileUrl = ${profileUrl} (owner = ${owner}) associatedContract = ${associatedContract}. chainId=${chainId}`,
            )
          }
          const profile = await repositories.profile.findOne({
            where: {
        ...
        if (err.code != 'BUFFER_OVERRUN' && err.code != 'INVALID_ARGUMENT') {
          // error parsing old event on goerli, and chainId mismatch
...
const syncProfileEvents = async (
  ...
  const log3 = await getProfileEvents(topics3, Number(chainId), chainProvider, profileAddress)
        ...
        if (evt.name === EventName.ExtendExpiry) {
          ...
          if (profile) {
            ...
            if (Number(timestamp) !== 0) {
              ...
              logger.info(
                `New ExtendExpiry event found. profileURL=${profileUrl} expireAt=${timestamp} chainId=${chainId}`,
              )
            }
          }
          ...
          if (
            from !== helper.AddressZero() &&
            to !== helper.AddressZero() &&
            ...
            if (profile) {
              ...
              if (!imageUrl) {
                imageUrl = await core.generateCompositeImage(profile.url, core.DEFAULT_NFT_IMAGE)
              }
              const toWallet = await repositories.wallet.findByChainAddress(`${chainId}`, ethers.utils.getAddress(to))
              if (!toWallet) {
                await repositories.profile.updateOneById(profile.id, {
                  ownerUserId: null,
                  ownerWalletId: null,
              ...
              await nftService.executeUpdateNFTsForProfile(profile.id, chainId.toString())
              logger.info(
                `New profile transfer event found. profileURL=${profile.url} from=${from} to=${to} chainId=${chainId}`,
              )
            }
          }
        }
