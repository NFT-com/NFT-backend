...
export const recordView = async (_: any, args: gql.MutationRecordViewArgs, _ctx: Context): Promise<boolean> => {
  const schema = Joi.object().keys({
    viewerId: Joi.string().required(),
    viewerType: Joi.string()
      .valid(...Object.values(gql.ViewerType))
      .required(),
    viewedId: Joi.string().required(),
    viewedType: Joi.string()
      .valid(...Object.values(entity.ViewableType))
      .required(),
  })
  ...
  const handleViewArgs = {
    ...args.input,
    viewerType:
      args.input.viewerType === gql.ViewerType.ProfileHolder
